...
int main(int argc, char* argv[])
{
  ...
  ofs.open("./ciphertxt.out"); 
  std::locale::global(loc);					      //恢复全局locale
  if (!ofs.is_open()){
    printf("文件打开失败！");
    //return;	//打开文件失败则结束运行
  }
  ofs << ctxt.writeToJSON();
  ofs.close();


  std::ifstream ifs;
  ifs.open("./ciphertxt.out");
  ctxt = helib::Ctxt::readFromJSON(ifs,public_key);
  ifs.close();
  //helib::JsonWrapper citxt;
  // std::cout<<citxt<<std::endl;
